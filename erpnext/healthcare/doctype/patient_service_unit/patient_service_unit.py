...
class PatientServiceUnit(Document):
	pass
# 	def validate(self):
...
# 		for child in self.assigned_users:
# 			if child.user :
# 				frappe.permissions.add_user_permission("Patient Service Unit", self.name, child.user)
#
...
# 	if users:
# 		frappe.throw(_("Patient Service Unit {0} is already assigned to User {1}").format(parent, user), frappe.DuplicateEntryError)
